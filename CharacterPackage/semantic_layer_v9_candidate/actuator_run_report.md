...
Next actuator: `leg_quad_loop_retopo_proxy`.

Reason: weapon and boots now have candidate hard-surface proxies. The next
largest blocker is continuous leg/knee/ankle structure, but the next pass should
still be a retopo proxy/spec actuator rather than full production retopology.
