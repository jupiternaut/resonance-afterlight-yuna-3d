...
def validate_cloth_candidate_report(report: dict[str, Any]) -> list[str]:
    ...
    if report.get("actuator") != "cloth_seam_surface_v0":
        errors.append("unexpected actuator")
    if report.get("status") not in {"generated_with_warnings", "manual_review_required", "failed"}:
        errors.append("status must be generated_with_warnings, manual_review_required, or failed")
    mesh = report.get("mesh_summary", {})
    if mesh.get("vertices", 0) <= 0:
        errors.append("mesh has no vertices")
    if mesh.get("faces", 0) <= 0:
    ...
    required_gate_fields = (
        ...
        "cloth_front_visual_candidate_status",
        "cloth_dcc_handoff_status",
    )
    for field in required_gate_fields:
        if field not in validation:
            errors.append(f"cloth candidate missing gate field: {field}")
    if validation.get("overlay_front_is_acceptance_gate") is not False:
        errors.append("cloth candidate must not use overlay_front as the acceptance gate")
    if validation.get("cloth_side_volume_present") is not True:
        errors.append("cloth candidate must keep side-view volume present")
    if validation.get("cloth_edge_thickness_present") is not True:
        errors.append("cloth candidate must expose edge thickness metadata")
    return errors
