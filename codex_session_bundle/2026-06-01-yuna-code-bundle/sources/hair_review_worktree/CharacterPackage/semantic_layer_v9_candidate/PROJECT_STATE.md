...
- `CharacterPackage/tools/separate_hair_debug_beauty_and_add_style_gate_v1.py`
...
- `replace_in_beauty_glb=false`
- `ready_for_cloth_seam_surface=false`
- Boundary: debug/planning overlays are now separated from beauty renders, but
  the beauty candidate is still construction-like and not accepted against the
  cinematic sci-fi heroine style target.
...
the style gate failed automatic acceptance. The candidate still needs manual
style review before it can influence any beauty replacement or unblock
`cloth_seam_surface`.
